...
class ModelSpeech(): # 语音模型类
	...
	def TrainModel(self, datapath, epoch = 2, save_step = 1000, batch_size = 32, filename = abspath + 'model_speech/m251/speech_model251'):
		'''
		训练模型
		参数：
			datapath: 数据保存的路径
	...
	def LoadModel(self,filename = abspath + 'model_speech/m251/speech_model251.model'):
		'''
		加载模型参数
		'''
	...
	def SaveModel(self,filename = abspath + 'model_speech/m251/speech_model251',comment=''):
		'''
		保存模型参数
		'''
		self._model.save_weights(filename+comment+'.model')
		self.base_model.save_weights(filename + comment + '.model.base')
		f = open('step251.txt','w')
...
if(__name__=='__main__'):
	...
	#ms.LoadModel(modelpath + abspath +  'm251/speech_model251_e_0_step_98000.model')
