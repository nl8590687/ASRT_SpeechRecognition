...
class DataSpeech():
	...
	def data_genetator(self, batch_size=32, audio_length = 1600):
		...
		while True:
			...
			for i in range(batch_size):
				t = threading.Thread(target = self.thread_get_data, args=(ran_num + i,))
				t.start()
				thread.append(t)
			...
			for i in range(batch_size):
				thread[i].join() # 线程阻塞，等待每个子线程运行完毕
			
			for i in range(batch_size):	
				assert not self.dataqueue.empty()
				[data_input, data_labels] = self.dataqueue.get()
	...
	def thread_get_data(self, i):
		if(self.LoadToMem == False):
			data_input, data_labels = self.GetData(i % self.DataNum)  # 从随机数开始连续向后取一定数量数据
			self.dataqueue.put([data_input, data_labels])
		else:
			data_input, data_labels = self.GetDataFromMem(i % self.DataNum)  # 从随机数开始连续向后取一定数量数据
			self.dataqueue.put([data_input, data_labels])
